...
#
# INERT (-no-apply): launched with -no-apply so this parallel instance SKIPS
# every state-mutating boot hook (nvram set/commit, daemon kills, iptables,
# hostapd/udhcpc spawn, supervisor reconcile) and makes ZERO writes to the
# live /tmp/webui-hapd. This is the fix for the br-0046 rejection, where a
# second webui ran its boot hooks against the hardcoded-shared hapd dir and
# tore down the live guest-BSS hostapds. The custom -conf (/data/br/webui)
# additionally instance-scopes the binary's hapd dir as defense-in-depth.
#
...
case "$1" in
    start)
        ...
        [ -x "$BIN" ] || exit 0
        mkdir -p "$CONFDIR"
        log "starting webui channel=$CHAN bin=$BIN version=$(webui_ver "$BIN") listen=$LISTEN no-apply=1"
        (
            N=0
            FAILS=0
            WSTART=$(date +%s)
            while [ $N -lt 20 ]; do
                "$BIN" -no-apply -listen "$LISTEN" -www "$WWW" -conf "$CONFDIR" >/dev/null 2>&1 &
                echo $! > "$PIDFILE"
                wait $!
                NOW=$(date +%s)
