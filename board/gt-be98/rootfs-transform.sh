...
# binary; launched by the S29 br-webui rail as a parallel listener.
...
# static-linkage guard: none of these may have PT_INTERP or DT_NEEDED.
...
echo "rootfs-transform: /usr/br harvest OK (busybox $(stat -c%s "$BRDST/bin/busybox")B + $(wc -l < "$WORK/bb.links") links, dropbearmulti $(stat -c%s "$BRDST/sbin/dropbearmulti")B, openssl $(stat -c%s "$BRDST/bin/openssl")B, webui $(stat -c%s "$BRDST/sbin/webui")B)"
